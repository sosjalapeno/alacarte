...
extern signed_size_type read(int fd, void *buf, size_type count);
...
static int is_resolver_space(char value) {
  return value == ' ' || value == '\t' || value == '\r';
}

static unsigned int parse_nameservers(
  char *config,
  size_type length,
  const char **servers,
  unsigned int capacity
) {
  static const char keyword[] = "nameserver";
  const size_type keyword_length = sizeof(keyword) - 1;
  size_type cursor = 0;
  unsigned int count = 0;

  while (cursor < length && count < capacity) {
    size_type line_end = cursor;
    while (line_end < length && config[line_end] != '\n') line_end++;
    const size_type next_line = line_end < length ? line_end + 1 : length;

    while (cursor < line_end && is_resolver_space(config[cursor])) cursor++;

    size_type matched = 0;
    while (
      matched < keyword_length &&
      cursor + matched < line_end &&
      config[cursor + matched] == keyword[matched]
    ) {
      matched++;
    }
    ...
    if (
      matched == keyword_length &&
      cursor + matched < line_end &&
      is_resolver_space(config[cursor + matched])
    ) {
      cursor += matched;
      while (cursor < line_end && is_resolver_space(config[cursor])) cursor++;

      const size_type address_start = cursor;
      while (
        cursor < line_end &&
        !is_resolver_space(config[cursor]) &&
        config[cursor] != '#'
      ) {
        cursor++;
      }
      ...
      if (cursor > address_start) {
        config[cursor] = '\0';
        servers[count++] = config + address_start;
      }
    }
...
int _resolv_set_nameservers_for_net(
  unsigned int net_id,
  const char **servers,
  unsigned int server_count,
  const char *domains
) {
  typedef int (*set_nameservers_fn)(
    unsigned int,
    const char **,
    unsigned int,
    const char *
  );
  static set_nameservers_fn real_set_nameservers;

  if (real_set_nameservers == 0) {
    real_set_nameservers = (set_nameservers_fn)dlsym(
      RTLD_NEXT,
      "_resolv_set_nameservers_for_net"
    );
  }
  if (real_set_nameservers == 0) return -1;

  char config[4097];
  const char *container_servers[4];
  const int fd = open("/etc/resolv.conf", O_RDONLY);
  if (fd >= 0) {
    const signed_size_type bytes_read = read(fd, config, sizeof(config) - 1);
    (void)close(fd);

    if (bytes_read > 0) {
      config[bytes_read] = '\0';
      const unsigned int container_server_count = parse_nameservers(
        config,
        (size_type)bytes_read,
        container_servers,
        (unsigned int)(sizeof(container_servers) / sizeof(container_servers[0]))
      );

      if (container_server_count > 0) {
        write_text("[.] using container DNS: ");
        for (unsigned int i = 0; i < container_server_count; i++) {
          if (i != 0) write_text(", ");
          write_text(container_servers[i]);
        }
        write_text("\n");
        return real_set_nameservers(
          net_id,
          container_servers,
          container_server_count,
          domains
        );
      }
    }
  ...
  write_text("[!] container DNS unavailable, using wrapper defaults\n");
  return real_set_nameservers(net_id, servers, server_count, domains);
}
...
int main(void) {
  char resolver_config[] =
    ...
    "  nameserver\t10.89.0.1 # podman\n"
    "options ndots:0\n";
  const char *servers[4] = { 0 };
  const unsigned int server_count = parse_nameservers(
    resolver_config,
    sizeof(resolver_config) - 1,
    servers,
    (unsigned int)(sizeof(servers) / sizeof(servers[0]))
  );
  assert(server_count == 2);
  assert(strcmp(servers[0], "127.0.0.11") == 0);
  assert(strcmp(servers[1], "10.89.0.1") == 0);
